...
GetSentiment <- function(file) {
  ...
  sntmnt <- tokens %>%
    inner_join(get_sentiments("nrc")) %>% # pull out only sentiment words ##227
...
if (!dir.exists("counted_sentiments/")) {
  dir.create("counted_sentiments/")
}

write_csv(sentiments, "./Data/Manipulated Data/sentiments2020-12-06.csv")
